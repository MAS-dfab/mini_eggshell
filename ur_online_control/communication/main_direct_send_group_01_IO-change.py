...
filename = os.path.join(path, "..", "commands_group_01_test.json")
...
io_list = data['io_list']
commands = format_commands(gh_commands, len_command)

# check length commands & stop_extruder
# if move_filament_loading_pt == True:
#     if len(stop_extruder) != len(commands)-2:
#         raise ValueError("Different number of stop_extruder and commands")
#     elif len(stop_extruder) != len(commands):
#         raise ValueError("Different number of stop_extruder and commands")
...
# UR SCRIPT
# ===============================================================
def movel_commands(server_address, port, tcp, commands, io_list, filament_extruder_motor_DO):
    script = ""
    script += "def program():\n"
    x, y, z, ax, ay, az = tcp
    ...
    for i in range(len(commands)):
        ...
        if io_list[i] == 0:
            script += "\tset_digital_out(%i, False)\n" % (int(filament_extruder_motor_DO))
            script += "\ttextmsg(\">> Turn I/O to OFF.\")\n"
        else:
            script += "\tset_digital_out(%i, True)\n" % (int(filament_extruder_motor_DO))
            script += "\ttextmsg(\">> Turn I/O to ON.\")\n"
...
def main(commands):
    ...
    #
    filament_extruder_motor_DO = 1
    # plastic_extruder_fan_DO = 0


    script = movel_commands(server_address, server_port, tool_angle_axis, commands, io_list, filament_extruder_motor_DO)
